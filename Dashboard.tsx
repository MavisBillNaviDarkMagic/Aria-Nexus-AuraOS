...
import { Cpu, Server, Clock, Activity, Zap, Smartphone, Shield, Globe, Github, Brain, Download, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ metrics, config }) => {
  const mockBuilds = [
    ...
    { id: 'B-940', status: 'FAILED', date: '2 días', ver: '6.7.0' },
  ];
  ...
  return (
    <div className="space-y-8 lg:space-y-12 pb-32">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 lg:gap-6">
        ...
        <StatusCard label="NEURAL" value={`${Math.round(metrics.neuralLoad)}%`} icon={Activity} color="text-cyan-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-10 relative">
            <div>
               <h3 className="text-3xl font-black text-white tracking-tighter">RED DE CONSCIENCIA REMOTA</h3>
               <p className="text-xs text-slate-500 font-bold uppercase tracking-[0.3em] mt-1">Sincronización de pulsos AuraOS</p>
            </div>
            <div className="flex gap-4">
              <div className="px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-black text-emerald-400 uppercase flex items-center gap-2">
                <Globe size={10} /> Sovereign Node Online
              </div>
            </div>
          </div>
        ...
        <div className="glass rounded-[3rem] p-10 flex flex-col gap-8 bg-gradient-to-br from-slate-950 to-black border-emerald-500/20 shadow-[0_0_50px_rgba(16,185,129,0.05)]">
           ...
           <div className="mt-auto p-6 rounded-2xl bg-fuchsia-500/5 border border-fuchsia-500/10 text-center">
             <div className="text-[10px] font-black text-fuchsia-400 tracking-widest uppercase mb-2">Próxima Compilación</div>
             <div className="text-sm font-mono text-slate-300">AUTO-SYNC: ENABLED</div>
           </div>
        </div>
      </div>
...
const HardwareLine: React.FC<{ label: string; value: string; icon: any }> = ({ label, value, icon: Icon }) => (
  <div className="flex items-center gap-5 p-6 glass rounded-[2rem] border-white/5">
